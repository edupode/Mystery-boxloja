...
# 1. Checkout - Número da Encomenda
...
# 3. Chat Admin - Aprovação/Rejeição
def test_admin_chat_sessions():
    ...
    try:
        ...
        if "agent_id" not in assigned_session or not assigned_session["agent_id"]:
        ...
        if not another_session_id:
            return log_test_result("Admin Reject Chat Session", False, "Failed to create chat session for rejection test")
        ...
        response = requests.get(f"{API_URL}/admin/chat/sessions", headers=headers)
        if response.status_code != 200:
            return log_test_result("Admin Get Sessions After Reject", False, f"Failed: {response.text}")
        
        sessions = response.json()
        rejected_session = next((s for s in sessions if s["id"] == another_session_id), None)
        
        if not rejected_session:
            return log_test_result("Admin Reject Chat Session", False, "Session not found after rejection")
        
        if rejected_session["status"] != "rejected":
            return log_test_result("Admin Reject Chat Session", False, f"Session status is {rejected_session['status']}, expected 'rejected'")
        
        log_test_result("Admin Reject Chat Session", True, "Session successfully rejected")
...
def test_product_image_upload():
    ...
    try:
        ...
        log_test_result("Verify Product Image", True, "Base64 image was correctly saved and prioritized")
